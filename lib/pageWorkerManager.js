...
var ActivatePageWorker = function(worker)
{
    ...
    if (pageWorkers.length > 1 && sitePinned == null)
    {
    ...
    sitePin = ActionButton({
        ...
        onClick: function(state) {
            if (sitePin.pinned)
            {
                ...
                sitePin.pinned = true;
                sitePin.label = `Media Keys: ${pageWorkers[pageWorkers.length - 1].tab.title}`;
                sitePin.icon = getFavicon(require("sdk/tabs").activeTab);
            }
            else
            {
